...
parsers = {
    ...
    137: [lambda x: struct.unpack('<BBB', bytearray(x)), None, lambda x: struct.unpack('<Q', bytearray(x))[0], lambda x: list(struct.pack('<BBB', *x))],
    138: [lambda x: struct.unpack('<B', bytearray(x)), None, lambda x: struct.unpack('<Q', bytearray(x))[0], lambda x: list(struct.pack('<B', *x))],
    # Calibration
