...
def move_block_to_location(bot3, from_pos, from_pos_above, to_pos, above_pos):
    bot3.move_to(*from_pos)  # Fixed syntax error
    bot3.suction_cup.suck()
    sleep(0.25)  # Short suction delay
    bot3.move_to(*from_pos_above)  # Ensure correct variable usage
    bot3.move_to(*above_pos)
    bot3.move_to(*to_pos)  # Move block to drop-off
    bot3.suction_cup.idle()  # Release block
    bot3.move_to(*above_pos)  # Return to initial above position
...
while True:
    if bot2.get_ir() == False:
        ...
        bot3.move_to(*start_position)  # Return to initial position
        bot3.move_to(*block_above)  # Move to block above position
        bot3.move_to(*block_pick)  # Move to block pick-up
        bot3.suction_cup.suck()  # Start suction
        sleep(0.25)  # Short delay for suction
        bot3.move_to(*block_above)  # Move above block
        bot3.move_to(*color_sensor_above)  # Move to color sensor position
        bot3.move_to(*color_sensor_on)  # Move to sensor on position
        bot3.suction_cup.idle()  # Idle suction cup
        bot3.move_to(*color_sensor_above)  # Move back to sensor above position
...
bot3.move_to(*start_position)  # Ensure correct return position
